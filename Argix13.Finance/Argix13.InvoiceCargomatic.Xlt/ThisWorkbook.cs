...
namespace Argix.Finance {
    ...
    public partial class ThisWorkbook {
        //Members
        private const string USP_DETAIL = "uspInvTsortClientInvoiceSortedFreightCargomaticGet",TBL_DETAIL = "ClientInvoiceTable";
        ...
        private void ThisWorkbook_Startup(object sender,System.EventArgs e) {
            ...
            try {
                ...
                string clid = "",invoice = "";
#if DEBUG
                clid = "116"; invoice = "333317900";    //Cargomatic: 116, 333317900
#endif
                if (cmd != null) {
        ...
        private void createDetailBody(InvoiceDataset.ClientInvoiceTableDataTable invoiceTable) {
            ...
            Excel.Range row0 = ws.Range[ws.Cells[ROW0_DETAIL + 1,1],ws.Cells[ROW0_DETAIL + 1,23]].EntireRow;
            ...
            object[,] values = new object[rowCount,23];
            for (int i = 0;i < rowCount;i++) {
                values[i,0] = "'" + invoiceTable[i].InvoiceNumber.Trim();
                values[i,1] = invoiceTable[i].InvoiceDate.ToString("MM/dd/yyyy");
                values[i,2] = "'" + invoiceTable[i].PRONumber.Trim();
                values[i,3] = "'" + (!invoiceTable[i].IsStoreNumberNull() ? invoiceTable[i].StoreNumber.ToString() : "");
                values[i,4] = "'" + invoiceTable[i].StoreName.Trim();
                values[i,5] = "'" + (!invoiceTable[i].IsStoreAddressLine1Null() ? invoiceTable[i].StoreAddressLine1.ToString() : "");
                values[i,6] = "'" + (!invoiceTable[i].IsStoreCityNull() ? invoiceTable[i].StoreCity.ToString() : "");
                values[i,7] = "'" + invoiceTable[i].StoreState.Trim();
                values[i,8] = "'" + invoiceTable[i].StoreZip.Trim();
                values[i,9] = invoiceTable[i].CtnQty;
                values[i,10] = invoiceTable[i].CartonRate;
                values[i,11] = invoiceTable[i].PltQty;
                values[i,12] = invoiceTable[i].PalletRate;
                values[i,13] = invoiceTable[i].Weight;
                values[i,14] = invoiceTable[i].RatedWeight;
                values[i,15] = invoiceTable[i].WeightRate;
                values[i,16] = invoiceTable[i].Surcharge;
                values[i,17] = invoiceTable[i].IsConsolidationChargeNull() ? 0.0m : invoiceTable[i].ConsolidationCharge;
                values[i,18] = invoiceTable[i].FuelRate;
                values[i,19] = invoiceTable[i].FuelSurcharge;
                values[i,20] = invoiceTable[i].DeliveryTotal;
                values[i,21] = "'" + invoiceTable[i].StoreBLNumber.Trim();
                values[i,22] = "'" + invoiceTable[i].MasterBLNumber.Trim();
            }
            ws.Range[ws.Cells[ROW0_DETAIL,1],ws.Cells[ROW0_DETAIL + rowCount - 1,23]].Value2 = values;
